    ...
    public class HubConnectionProtocolTests
    {
        ...
        public async Task StreamFailsIfCompletionMessageIsNotStreamCompletionMessage()
        {
            ...
            try
            {
                ...
                Assert.Equal(
                    "Unexpected message type received. Expected message type: 'StreamCompletionMessage', actual message type: 'CompletionMessage'.",
                    ex.Message);
            }
            finally
            {
        ...
        public async Task InvokeFailsWithErrorWhenStreamCompletionReceived()
        {
            ...
            try
            {
                ...
                var ex = await Assert.ThrowsAsync<InvalidOperationException>(() => invokeTask).OrTimeout();
                Assert.Equal(
                    "Unexpected message type received. Expected message type: 'CompletionMessage', actual message type: 'StreamCompletionMessage'.",
                    ex.Message);
            }
            finally
            {
