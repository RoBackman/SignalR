    ...
    internal abstract class InvocationRequest : IDisposable
    {
        ...
        private class Streaming : InvocationRequest
        {
            ...
            public override void Complete(HubMessage message)
            {
                ...
                {
                    Logger.ReceivedUnexpectedCompletion(InvocationId);
                    Fail(new InvalidOperationException(
                        $"Unexpected message type received. Expected message type: '{nameof(StreamCompletionMessage)}', actual message type: '{message.GetType().Name}'."));
                    return;
                }
        ...
        private class NonStreaming : InvocationRequest
        {
            ...
            public override void Complete(HubMessage message)
            {
                ...
                {
                    Logger.ReceivedUnexpectedStreamCompletion(InvocationId);
                    Fail(new InvalidOperationException(
                        $"Unexpected message type received. Expected message type: '{nameof(CompletionMessage)}', actual message type: '{message.GetType().Name}'."));
                    return;
                }
