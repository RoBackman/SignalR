...
namespace Microsoft.AspNetCore.SignalR.Internal.Protocol
{
    public class MessagePackHubProtocol : IHubProtocol
    {
        ...
        {
            Debug.Assert(arraySize == 3 || arraySize == 4, "Unexpected item count");

            var invocationId = ReadInvocationId(unpacker);
            // Error is optional so StreamCompletion without error will has 3 items, StreamCompletion with error has 4 items
            var error = arraySize == 4 ? ReadString(unpacker, "error") : null;
        ...
        private void WriteStreamCompletionMessage(StreamCompletionMessage streamCompletionMessage, Packer packer)
        {
            ...
            packer.PackArrayHeader(3 + (hasError ? 1 : 0));
